...
definition xor :: gate
where "xor x y \<equiv> (x \<or> y) \<and> (\<not> (x \<and> y))"
...
definition nor :: gate
where "nor x y \<equiv> \<not>(x \<or> y)"
...
definition xnor :: gate
where "xnor x y \<equiv> (x \<and> y) \<or> \<not>(x \<or> y)"
...
where "nand x y \<equiv> (\<not>(x \<and> y))"
